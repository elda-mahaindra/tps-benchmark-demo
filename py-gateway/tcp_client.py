...
"""
TCP Client for py-switching Communication

Handles TCP communication with py-switching service using the same protocol
as py-switching TCP server (length-prefixed JSON messages).
"""
...
class PySwitchingTCPClient(object):
    """TCP client for communicating with py-switching service"""
    ...
    def __init__(self, host, port, config):
        ...
        Args:
            host (str): py-switching host
            port (int): py-switching port
    ...
    def connect(self):
        """Establish connection to py-switching"""
        try:
            ...
            logger.info("Connected to py-switching at {}:{}".format(self.host, self.port))
            return True
            
        except Exception as e:
            logger.error("Failed to connect to py-switching: {}".format(str(e)))
            self.is_connected = False
            return False
    
    def disconnect(self):
        """Disconnect from py-switching"""
        ...
        logger.info("Disconnected from py-switching")
    
    def send_request(self, request_data, timeout=30):
        """
        Send request to py-switching and wait for response
        ...
        if not self.is_connected:
            if not self.connect():
                raise Exception("Not connected to py-switching")
    ...
    def _send_message(self, data):
        """Send a message to py-switching"""
        try:
            # Convert to JSON
            ...
            logger.debug("Sent request to py-switching: {}".format(json_data))
            
        except Exception as e:
            logger.error("Failed to send message to py-switching: {}".format(str(e)))
            raise e
    
    def read_responses(self):
        """Continuously read responses from py-switching"""
        while not self.shutdown_event.is_set() and self.is_connected:
            try:
                ...
                if not length_bytes:
                    logger.info("Connection closed by py-switching")
                    break
                ...
                logger.error("Error reading response from py-switching: {}".format(str(e)))
                break
    ...
    def _process_response(self, response):
        """Process response from py-switching"""
        try:
            # Extract message ID
