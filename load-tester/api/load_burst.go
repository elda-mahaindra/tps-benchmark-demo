...
func (api *Api) loadBurst(c *fiber.Ctx) error {
	// Parse request configuration
	var param service.LoadBurstParam
	...
	// Validate parameters
	if err := api.validateLoadBurstParam(&param); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(map[string]any{
	...
	// Call service layer
	result, err := api.service.LoadBurst(context.Background(), &param)
	if err != nil {
