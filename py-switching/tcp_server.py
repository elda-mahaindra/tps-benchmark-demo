...
"""
py-switching TCP Server - Persistent Connection Handler

Implements TCP server with persistent connections following the same pattern
as py-core TCP server. Handles multiple concurrent requests on a single connection
using message ID correlation.
"""
...
class TCPServer(object):
    ...
    def start(self):
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("py-switching TCP server starting...")
            ...
            logger.info("Target py-core: {}:{}".format(
                self.config['external_service']['py_core']['host'],
                self.config['external_service']['py_core']['port']
            ))
            logger.info("=" * 60)
